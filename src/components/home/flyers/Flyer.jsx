...
function Flyer() {
    ...
    return (
        <div className='flex flex-col justify-between gap-4 md:flex-row'>
            {post.map((post,index) => (
                <div key={index} className="lg:w-1/2 sm:w-full w-full max-w-full bg-cover bg-center overflow-hidden lg:h-80 inset-0 transform scale-100 hover:scale-105 transition-transform duration-500 ease-in-out bg-black bg-opacity-35" style={{ backgroundImage: `url(${post.imageSrc})` }}>
