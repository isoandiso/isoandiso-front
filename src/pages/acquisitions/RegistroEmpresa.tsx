...
function RegistroEmpresa(props) {
  ...
  const {user,handleViewChange,getUser} = props;
  ...
  async function _submit(event){
    ...
    if([ruc, socialReason, province, city, address, economicActivity, economicSector, companySize].every((field) => field.trim() !== ""))
    {
        ...
        const rucResponse = await apiCallsCompany._updateRuc(user._id,ruc.trim());
        const socialReasonResponse = await apiCallsCompany._updateSocialReason(user._id,socialReason.trim());
        const provinceResponse = await apiCallsCompany._updateProvince(user._id,province.trim());
        const cityResponse = await apiCallsCompany._updateCity(user._id,city.trim());
        const addressResponse = await apiCallsCompany._updateAddress(user._id,address.trim());
        const economicActivityResponse = await apiCallsCompany._updateEconomicActivity(user._id,economicActivity.trim());
        const economicSectorResponse = await  apiCallsCompany._updateEconomicSector(user._id,economicSector.trim());
        const companySizeResponse = await apiCallsCompany._updateCompanySize(user._id,companySize);
        //creamos y agregamos la sede principal
        const mainSite = await apiCallsCompanySite._createCompanySite("Sede principal",address,city,province);
        const updatedCompany = mainSite ? await apiCallsCompany._addSedeIdToCompany(user._id,mainSite._id) : null;
              ...
              <form onSubmit={_submit}>
                        ...
                        <div style={{padding: '15px'}}>
                          ...
                          <div style={{display:'flex', flexDirection: 'column'}}>
                            ...
                            <div><label>País:</label><input required type="text" className={Styles.inputDecorated} placeholder={user.countryId.name} disabled/></div>
