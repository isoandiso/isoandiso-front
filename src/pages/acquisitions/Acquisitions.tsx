...
function Acquisitions() {
  ...
  useEffect(()=>{
    async function setIsos(){
      const isoRRHH = await apiCallsIso._getIsoByNameStartWith("RRHH");
      const iso9001 = await apiCallsIso._getIsoByNameStartWith("ISO 9001");
      setIsoRRHH(isoRRHH);
      setIso9001(iso9001);
    }
    setIsos();
  ...
  const renderView = () => {
    switch (currentView) {
      case 'compra':
        return <Compra user={user} getUser={getUser}/>;
      case 'sistemas adquiridos':
        return <SistemasAdquiridos user={user}/>;
      case 'registro empresa':
        return <RegistroEmpresa user={user} getUser={getUser} handleViewChange={handleViewChange}/>;
      case 'sedes':
          return <Sedes user={user} getUser={getUser}/>;
      case 'areas/cargos':
        return <AreasCargos user={user} getUser={getUser}/>;
      case 'asignación de sistemas digitales':
        return <AsignaciónSistemasDigitales user={user} getUser={getUser}/>;
      case 'responsables de los sistemas digitalizado':
        return <ResponsablesDelSistemaDigital user={user} getUser={getUser}/>;
      default:
        return <div></div>;
    }
  };
      ...
      <div style={{padding:'2px'}}>
        <div style={{display: 'flex', backgroundColor:'white'}}>
            <div style={{width:'22.5%',borderRight:'ridge',textAlign:'center'}}>
                    ...
                    <div style={{borderBottom: '1px',borderStyle:'inset'}}>
                      <p 
                        style={{ 
                          ...
                          color: user.ruc===null ? (selectedOption === 'registro empresa' ? 'white' : 'black') : 'gray',
                          opacity:user.ruc===null ? '1':'0.5',
                          fontWeight: selectedOption === 'registro empresa' ? 'bolder' : 'inherit'
                        }} 
                        {...(user.ruc===null ? { onClick: () => handleViewChange('registro empresa')} : {})}
                      >
                        REGISTRO EMPRESA
                      </p>
                    </div>
                ...
                {
                  ((user.areaIds.length > 0) && (user.areaIds.some(area=> area.isoIds.length > 0)))
                  &&
                  <div style={{borderBottom: '1px',borderStyle:'inset'}}>
                    <p 
                      style={{ 
                      ...
                      onClick={async () => {
                        ...
                        if(!(((user.areaIds.some(area=>area.isoIds.some(iso=>iso.name.startsWith("RRHH")))) && (user.areaIds.some(area=>area.isoIds.some(iso=>iso.name.startsWith("ISO 9001"))))))){
                            await Swal.fire({
                                icon: 'warning',
                            ...
                            handleViewChange('responsables de los sistemas digitalizado');
                        }
                      }}
                    >
