...
export const apiCallsCompany = {
  async _updateCompanyCountry(companyId, countryId) {
    try {
  ...
  async _addAcquisitionIdToCompany(companyId, acquisitionId) {
    try {
  ...
  async _updateRuc(companyId,ruc) {
    try {
  ...
  async _updateSocialReason(companyId,socialReason) {
    try {
  ...
  async _updateProvince(companyId,province) {
    try {
  ...
  async _updateCity(companyId,city) {
    try {
  ...
  async _updateAddress(companyId,address) {
    try {
  ...
  async _updateEconomicActivity(companyId,economicActivity) {
    try {
  ...
  async _updateEconomicSector(companyId,economicSector) {
    try {
  ...
  async _updateCompanySize(companyId,companySize) {
    try {
  ...
  async _addSedeIdToCompany(companyId,companySiteId) {
    try {
  ...
  async _addAreaIdToCompany(companyId,companyAreaId) {
    try {
  ...
  async _createWorker(workerData){
    try {
        const { data } = await api.post('/company/createWorker', workerData);
        return data;
      } catch (error) {
...
export const apiCallsCompanyCountry = {
...
export const apiCallsCompanyAcquisitionType = {
...
export const apiCallsCompanyAcquisition = {
  async _createAcquisition(isos:Iso[], acquisitionTypeId) {
...
export const apiCallsCompanySite = {
  ...
  async _deleteCompanySite(siteId) {
      try {
...
export const apiCallsCompanyArea = {
...
async _deleteCompanyArea(areaId) {
  try {
...
async _deleteIsosOfArea(areaId) {
  try {
...
async _deleteWorkerOfArea(areaId) {
  try {
      await api.delete(`/companyArea/${areaId}/deleteWorker`);
      await Swal.fire({
          icon: 'success',
...
async _updateResponsibleWorker(areaId,responsibleWorkerId) {
  try {
    const { data } = await api.put(`/companyArea/${areaId}/updateResponsibleWorker`, { responsibleWorkerId: responsibleWorkerId });
    return data;
  } catch (error) {
...
export const apiCallsWorkerNationality = {
  async _getNationalities(){
    try {
        const { data } = await api.get('/workerNationality');
        return data;
      } catch (error) {
